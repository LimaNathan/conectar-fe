...
class RemoteClientsRepository implements ClientsRepository {
  ...
  Future<Result<ClientDTO>> create(CreateClientDTO client) async {
    try {
      final json = client.toJson()..remove('runtimeType');

      log(json.toString(), name: 'create');
      final result = await _api.post(Endpoints.createClient, data: json);
  ...
  Future<Result> delete(int clientId) async {
    try {
      await _api.delete(Endpoints.deleteClient, queryParms: {'id': clientId});
  ...
  @override
  Future<Result<ViaCepResultDto>> findByZIPCode(String zip) async {
    final sanitized = zip.replaceAll(RegExp(r'[^0-9]'), '');

    if (sanitized.length != 8) return Error(AppException('CEP Inválido'));

    try {
      final response = await _api.get(Endpoints.findByZIPCode(zip));
      final json = Map<String, dynamic>.from(response.data);

      return Ok(ViaCepResultDto.fromJson(json));
    } on DioException catch (e) {
      return Error(AppException(e.message ?? 'Houve um erro inesperado'));
    } catch (e) {
      return Error(e.toString());
